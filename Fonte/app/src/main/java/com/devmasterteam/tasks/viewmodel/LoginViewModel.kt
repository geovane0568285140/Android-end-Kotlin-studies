...
class LoginViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun doLogin(email: String, password: String) {
       ...
       person.personLogin(email, password, object : APIListener<PersonModel>{
           ...
           override fun onSuccess(model: PersonModel) {
               ...
               securityPreferences.store(TaskConstants.SHARED.TOKEN_KEY, model.personKey)
               securityPreferences.store(TaskConstants.SHARED.PERSON_NAME, model.name)

               RetrofitClient.addHeards(model.token, model.personKey)
    ...
    fun verifyLoggedUser() {
        ...
        RetrofitClient.addHeards(token, person)
